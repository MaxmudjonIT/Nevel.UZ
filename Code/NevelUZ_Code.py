...
from NevelUZ_Data import db_cursor, db_connect
...
# CHANNEL_ID = '@Test_Channel_Bot'  # или 'your_channel_id' в формате целого числа
...
#         await bot.send_message(user_id, "Здравствуйте! Давайте для начала выберем язык обслуживания!\n\n"
...
#         ])
#         await message.answer("Пожалуйста, подпишитесь на наш канал, чтобы пользоваться ботом.", reply_markup=keyboard)
#
#
...
#                                         "Hi! Let's first we choose language of serving!\n\n", reply_markup=language)
#     else:
#         await bot.answer_callback_query(callback_query.id, "Вы все еще не подписаны.")
#
#
# async def check_bot_admin_status():
#     try:
#         bot_member = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=(await bot.get_me()).id)
#         logging.info(f"Bot status in {CHANNEL_ID}: {bot_member.status}")
#         if bot_member.status in ['administrator', 'creator']:
#             logging.info("Bot has admin rights.")
#         else:
#             logging.warning("Bot does not have admin rights.")
#     except TelegramBadRequest as e:
...
async def add_quantity(callback: types.CallbackQuery):
    ...
    await callback.message.answer_photo(photo=FSInputFile(Product69.photo),
                                        caption=f"{Product69.title}\n\n{Product69.description}\n\n{round(Product69.price, 2)}00 "f"сум\n\nКоличество - {Product69.quantity} \n",
                                        reply_markup=product69_keyboard)
...
if __name__ == '__main__':
    # dp.startup.register(check_bot_admin_status)
    asyncio.run(main())
